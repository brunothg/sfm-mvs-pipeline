...
namespace photogrammetrie {
    ...
    const AppLogger UnorderedFeatureMatchingStrategy::logger = AppLogger("UnorderedFeatureMatchingStrategy");

    UnorderedFeatureMatchingStrategy::UnorderedFeatureMatchingStrategy() = default;

    void
    UnorderedFeatureMatchingStrategy::calculateShotMatches(const Scene &scene, cv::Ptr<cv::DescriptorMatcher> &matcher,
        ...
        for (size_t iLeft = 0; iLeft < shots.size(); iLeft++) {
            for (size_t iRight = iLeft + 1; iRight < shots.size(); iRight++) {
...
#pragma omp parallel for default(none) shared(matchPairs, shots, scene, returnMatches, matcher, max, completed)
        for (auto &matchPair : matchPairs) {
            ...
            UnorderedFeatureMatchingStrategy::logger.debug(
...
#pragma omp critical
            ...
            completed++;
            UnorderedFeatureMatchingStrategy::logger.info(
